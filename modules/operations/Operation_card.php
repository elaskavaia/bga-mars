...
class Operation_card extends AbsOperation {
    function effect(string $color, int $inc): int {
        ...
        if ($payment_inst->isVoid()) {
            throw new BgaUserException(clienttranslate("Insufficient resources for payment"));
        }
    ...
    function getPrompt() {
        $delta = $this->getDelta();
        if ($delta >= 20) {
            return clienttranslate('${you} must select a card to play (ignore global requirements)');
        }
