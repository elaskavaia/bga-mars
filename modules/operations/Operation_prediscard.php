...
/**
 * Keep selected cards and discard the rest from draw and draft areas
 */
    ...
    function effect(string $color, int $inc): int {
        ...
        if ($count) {
            $this->game->notifyWithName(
                "message",
                clienttranslate('${player_name} keeps ${count} card/s'),
                [
                    "count" => $count,
                ],
                $this->getPlayerId()
        ...
        $this->game->undoSavepointWithLabel(clienttranslate("discard"), MA_UNDO_NOBARRIER);
        return 1;
    }
