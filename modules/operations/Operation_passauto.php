...
class Operation_passauto extends AbsOperation {
    ...
    function effect(string $color, int $inc, ?array $args = null): int {
        ...
        if (!$this->game->isPlayerAlive($player_id)) {
            throw new \Bga\GameFramework\VisibleSystemException("Pass operation is impossible in this state");
        }
        $isMulti = $this->game->isInMultiplayerMasterState();
        ...
        $this->game->dbSetTokenState("tracker_passed_$color", 2, "", ["_private" => true], $this->getPlayerId());
        $this->game->notifyWithName(
            "message_warning",
            clienttranslate("Advanced Pass scheduled for next turn"),
            ["_private" => true],
            $this->getPlayerId()
        );
        return 1;
    ...
    function getPrompt() {
        ...
        if ($this->game->getActivePlayerColor() !== $color) {
            return clienttranslate('${you} will Pass for this GENERATION on your next turn');
        } else {
            return clienttranslate(
                '${you} must confirm Pass for this GENERATION on your next turn, you can finish your current turn normally'
            );
        }
    }
