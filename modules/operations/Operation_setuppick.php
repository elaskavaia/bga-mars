    ...
    function effect(string $color, int $inc): int {
        ...
        foreach ($card_ids as $card_id) {
            if (startsWith($card_id, "card_corp")) {
                if ($corpmoney > 0) {
                    $this->game->userAssertTrue(totranslate("You can only select one corporation"));
                }
                ...
                $this->game->effect_moveCard(
                    ...
                    $card_id,
                    "hand_$color",
                    MA_CARD_STATE_SELECTED,
                    clienttranslate('You choose corporation ${token_name}'),
                    [
                        "_private" => true,
                    ]
                );
        ...
        if ($corpmoney == 0) {
            $this->game->userAssertTrue(totranslate("You must select one corporation"));
        }
        if ($this->game->isPreludeVariant() && $prelude != 2) {
            $this->game->userAssertTrue(totranslate("You must select exactly 2 Prelude cards"));
        }
        $cost = 3;
        $count = 0;
        ...
        foreach ($card_ids as $card_id) {
            if (startsWith($card_id, "card_main")) {
                ...
                $this->game->effect_moveCard(
                    ...
                    "hand_$color",
                    MA_CARD_STATE_SELECTED,
                    clienttranslate('You buy ${token_name}'),
                    [
                        "_private" => true,
                    ]
                );
        ...
        foreach ($card_ids as $card_id) {
            if (startsWith($card_id, "card_prelude")) {
                ...
                $this->game->effect_moveCard(
                    ...
                    "hand_$color",
                    MA_CARD_STATE_SELECTED,
                    clienttranslate('You got ${token_name}'),
                    [
                        "_private" => true,
                    ]
                );
        ...
        if ($count == 0) {
            ...
            $this->game->notifyPlayer(
                $this->getPlayerId(),
                "message_warning",
                clienttranslate("You did not select any initial project cards, it may be not a good idea. Undo if not too late"),
                []
            );
        }
        return 1;
