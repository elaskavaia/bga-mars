...
class Operation_tile extends AbsOperationTile {
    ...
    function checkPlacement($color, $ohex, $info, $map) {
        ...
        if ($reservename) {
            ...
            } elseif (array_search($ohex, $reshexes) === false) {
                return MA_ERR_NOTRESERVED;
            }
        } else {
            if (isset($info["reserved"])) {
        ...
        if ($this->checkMandatoryEffect($color, $ohex)) {
    ...
    protected function debugtilecretae() {
        ...
        if (!$tileid) {
            throw new BgaSystemException("Cannot get context for tile placement operation");
        }
    ...
    function effect(string $owner, int $inc): int {
        $tileid = $this->getTileId();
        if (!$tileid) {
            throw new BgaSystemException("Cannot get context for tile placement operation $tileid");
        }
        ...
        if ($this->getTileType() == MA_TILE_MINING) {
            ...
            if ($pp) {
                $this->game->putInEffectPool($owner, $pp, $tile);
            } else {
                throw new BgaVisibleSystemException("Location does '$ohex' not have resource bonus '$pp' foe tile $tile");
            }
        }
