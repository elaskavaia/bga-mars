...
class Operation_nmM extends AbsOperation {
    ...
    function argPrimaryDetails() {
        ...
        foreach ($alltypes as $type) {
            ...
            if ($er > 1) {
                ...
                if ($overres > $propres) {
                    $this->addProposal($info, [$type => $overres]);
                }
            }
    ...
    function canResolveAutomatically() {
        ...
        if (count($possible) == 1) {
            return false;
        } // this is only Custom option

        $info = $this->getStateArg("info");
        if ($info["payment"]["rescount"][$this->getType()] == 0) {
            return false;
        } // no money, force choice
        $alltypes = $this->getTypes();
        $uniqueRes = 0;
        foreach ($alltypes as $type) {
        ...
        if (count($possible) == 2 && $uniqueRes == 1) {
            return true;
        } // custom + a singe choice, means other resources are at 0
        return false;
    }
    ...
    function effect(string $owner, int $inc): int {
        ...
        if ($value == "payment") {
            ...
            if (!$uservalue) {
                throw new BgaUserException("Expecting payment parameter");
            }
            if (!is_array($uservalue)) {
                throw new BgaUserException("Expecting payment parameter to be array $uservalue");
            }
    ...
    protected function payWithResource($type, $ut, $infores) {
        ...
        if (!$tt) {
            $this->game->systemAssertTrue("Invalid payment type $type");
        } // XSS
        ...
        if ($ut > 0 && $ut <= $tt) {
            ...
            $message = sprintf(
                $this->game->_("Invalid amount of %s used for payment: %d of %d (max)"),
                $this->game->getTokenName($type),
                $ut,
                $tt
            );
