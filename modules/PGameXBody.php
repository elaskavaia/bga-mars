...
abstract class PGameXBody extends PGameMachine {
    ...
    protected function initTables() {
        try {
            ...
            if ($colonies) {
                ...
                // will discard one later
                else {
                    $coloniesNum = $numPlayers + 2;
                }
    ...
    function getNonReservedHexes() {
        ...
        foreach ($this->token_types as $key => $info) {
            if (startsWith($key, "hex_")) {
                if (array_get($info, "reserved")) {
                    continue;
                }
                if (array_get($info, "ocean")) {
                    continue;
                }
                $nonreserved[] = $key;
            }
    ...
    function findCard($num) {
        if (is_numeric($num)) {
            ...
            if (!array_get($this->token_types, $token)) {
                ...
                if (!$token) {
                    throw new feException("card not found $num");
                }
            }
        }
        return $token;
    ...
    function getAdjecentHexes($coords, $valid_coords = null) {
        ...
        $dx = ($y % 2) - 1;
        $trylist = [[$x + $dx, $y - 1], [$x + $dx + 1, $y - 1], [$x - 1, $y], [$x + 1, $y], [$x + $dx, $y + 1], [$x + $dx + 1, $y + 1]];
        ...
        foreach ($this->token_types as $key => &$info) {
            if (startsWith($key, "card_") || startsWith($key, "hex_")) {
                ...
                foreach ($expr_keys as $field) {
                    ...
                    try {
                        if ($r) {
                            $info["expr"][$field] = OpExpression::arr($r);
                        }
                    } catch (Exception $e) {
                ...
                try {
                    if ($r) {
                        $info["expr"][$field] = MathExpression::arr($r);
                    }
                } catch (Exception $e) {
                    $this->error("error while parsing $field $r");
                    $this->error($e);
            ...
            if (startsWith($key, "hex_")) {
                ...
                } elseif (endsWith($name, "Mons") || endsWith($name, "Tholus")) {
                    $info["vol"] = 1;
                }
    ...
    function evaluateTerm($x, $owner, $context = null, ?array $options = null) {
        switch ($x) {
            ...
            case "res":
                if ($context) {
                    $this->tokens->countTokensInLocation("$context");
                }
    ...
    function getNextReadyPlayer($player_id, $previous = false) {
        ...
        } // fallback if we lost player id due to zombie
        $this->systemAssertTrue("invalid player id", $this->isRealPlayer($player_id));
    ...
    function collectListeners($owner, $events, $card_context = null, string $extracontext = null) {
        ...
        foreach ($cards as $info) {
            ...
            if ($card === $extracontext) {
                $contextcardinplay = true;
            }
        }
        $res = [];
    ...
    function mtMatchEvent($trigger_rule, $trigger_owner, $event, $event_owner, &$splits = []) {
        ...
        foreach ($args as $arg) {
            ...
            if ($trigger_owner === $event_owner || $all === "any") {
                // for now only listen to own events
    ...
    function setCurrentStartingPlayer(int $playerId) {
        ...
        if (!$this->isSolo()) {
            $this->dbSetTokenLocation(
                ...
                "tableau_$color",
                0,
                clienttranslate('${player_name} is starting player for this generation'),
                [],
                $playerId
            );
        }
    ...
    function getLastGeneration() {
        ...
        if ($this->isPreludeVariant()) {
            $maxgen -= 2;
        } // Prelude sole ends with 12 generations not 14
        if ($this->getMapNumber() == MA_OPTVALUE_MAP_AMAZONIS_PLANITIA && $this->getGameStateValue("var_solo_flavour") == 0) {
            $maxgen += 2;
        } // This map is not design for solo mode really
        return $maxgen;
    }
    ...
    function switchActivePlayerIfNeeded($player_color, $op) {
        ...
        if ($active_player != $player_id) {
            ...
            if ($stage != MA_STAGE_GAME) {
                $switch = true;
            } elseif ($player_id == $master) {
                $switch = true;
            } elseif (!$canauto) {
                $switch = true;
            } elseif ($this->isZombiePlayer($active_player)) {
                $switch = true;
            }
    ...
    function effect_alteratingDraw($numcards, $location) {
        ...
        if ($count < count($players) * $numcards) {
            ...
            if ($numcards) {
                $this->notifyAllPlayers("message_warning", clienttranslate("Not enought cards to draw, splitting equally"), []);
            } else {
                $this->notifyAllPlayers("message_warning", clienttranslate("No cards left"), []);
            }
        }
    ...
    function effect_queueMultiDrawSetup($numcards = 10, $corps = 2, $prelude = 0) {
        ...
        foreach ($players as $player_id => $player) {
            ...
            if ($prelude > 0) {
                $this->tokens->pickTokensForLocation($prelude, "deck_prelude", "draw_$color");
            }
        }

        foreach ($players as $player_id => $player) {
            $color = $player["player_color"];
            if ($corps + $numcards + $prelude) {
                $this->multiplayerqueue($color, "setuppick");
            }
        }
    }
    ...
    function getReason(string $data) {
        ...
        if (!$type) {
            return $game->getTokenName($context);
        }
    ...
    function effect_draw($color, $deck, $to, $inc) {
        ...
        $this->dbSetTokensLocation(
            ...
            $to,
            null,
            clienttranslate('${player_name} draws ${token_names}'),
            [
                "_private" => true,
                "place_from" => $deck,
            ],
    ...
    function effect_untap($cardid) {
        ...
        } elseif (isset($rules["a"])) {
            $state = MA_CARD_STATE_ACTION_UNUSED; // activatable cards
            $this->dbSetTokenState($cardid, $state, "");
        } elseif (isset($rules["e"]) && $rules["t"] == MA_CARD_TYPE_EVENT) {
    ...
    function effect_finalScoring(): int {
        ...
        if ($this->getTerraformingProgression() >= 100) {
            $this->notifyAllPlayers("message", clienttranslate('It is the end of the generation ${gen} and Mars is terraformed!'), [
                "gen" => $gen,
            ]);
        } else {
            $this->notifyAllPlayers(
                "message",
                clienttranslate('It is the end of the generation ${gen} and Mars is sadly not terraformed'),
                ["gen" => $gen]
            );
        }
        ...
        foreach ($players as $player_id => $player) {
            ...
            $this->notifyMessage(
                clienttranslate('${player_name} has ${count} M€ left (for tiebreaker purposes)'),
                ["count" => $mc],
                $player_id
            );
    ...
    function scoreAll(array &$table = null) {
        ...
        if ($table !== null) {
            ...
            foreach ($players as $player_id => $player) {
                ...
                $this->notifyWithName("score", "", ["player_score" => 0, "inc" => 0, "mod" => 0, "noa" => 1], $player_id);

                $curr = $this->tokens->getTokenState("tracker_tr_{$color}");
                $this->dbIncScoreValueAndNotify(
                    $player_id,
                    $curr,
                    clienttranslate('${player_name} scores ${inc} point/s for Terraforming Rating'),
                    "",
                    [
        ...
        if (!$this->isSolo()) {
            ...
            if (count($markers) == 0) {
                ...
                foreach ($markers as $id => $rec) {
                    ...
                    if ($this->getRulesFor($loc, "rank", 10) == 1) {
    ...
    function scoreMilestone(string $loc, string $id, array &$table = null) {
        ...
        if ($commit) {
            $this->dbIncScoreValueAndNotify(
                $player_id,
                $points,
                clienttranslate('${player_name} scores ${inc} point/s for milestone'),
                "game_vp_ms",
                [
                    "target" => $loc,
                ]
        ...
        foreach ($scores as $color => $res) {
            ...
            if ($commit) {
                $this->dbIncScoreValueAndNotify(
                    $player_id,
                    $points,
                    clienttranslate('${player_name} scores ${inc} point/s for award ${award_name} with max value of ${award_counter}'),
                    "game_vp_award",
                    [
    ...
    function getBuyCardCost($color) {
        if ($this->getOwnCardOnTableau("card_corp_26", $color)) {
            return 5;
        }
        return 3;
    }
    ...
    function scoreMap(string $owner, array &$table = null) {
        ...
        foreach ($map as $hex => $info) {
            ...
            } elseif ($tt == MA_TILE_FOREST) {
                if ($commit) {
                    $this->dbIncScoreValueAndNotify($player_id, 1, "", "game_vp_forest", ["target" => $hex]);
                }
                $greenery += 1;
        ...
        if ($commit) {
            $this->notifyWithName(
                "message",
                clienttranslate('${player_name} scores ${inc} points for Greenery tiles'),
                [
                    "inc" => $greenery,
                ],
                $player_id
    ...
    function scoreCards(string $owner, array &$table = null) {
        ...
        if ($commit) {
            $this->notifyMessage(
                clienttranslate('${player_name} scores total ${inc} points for cards with fixed points'),
                [
                    "inc" => $vpdirect,
                ],
                $player_id
    ...
    function notifyTokensUpdate($player_id) {
        $this->notifyAllPlayers(
            "tokensUpdate",
            "",
            $this->getTokensUpdate($player_id) + [
                "player_id" => $player_id,
            ]
        );
    }
    ...
    function undoSavepointWithLabel($label = "", $barrier = 1) {
        ...
        if ($barrier) {
            $this->undoSavepointMeta["barrier"] = 1;
        }
        $move_id = $this->getNextMoveId();
        $player_id = $this->getActivePlayerId();
        if ($this->isUndoSavepoint()) {
            // already set, move on
            $this->dbMultiUndo->notifyUndoMove($this->undoSavepointMeta + ["move_id" => $move_id, "player_id" => $player_id]);
        } else {
            if ($this->isMultiActive()) {
                $this->dbMultiUndo->notifyUndoMove(["move_id" => $move_id, "player_id" => $player_id, "label" => "undo skipped"]);
                return;
            }
    ...
    function upgradeTableDb($from_version) {
        if ($from_version <= 2408211710) {
            // where your CURRENT version in production has number YYMMDD-HHMM
